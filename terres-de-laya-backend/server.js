...
app.post('/api/register-user', async (req, res) => {
  ...
  try {
    // Get the token first
    const tokenResponse = await axios.post('https://terresdelaya.eu.auth0.com/oauth/token', {
    ...
    // Register the user
    const userResponse = await axios.post('https://terresdelaya.eu.auth0.com/api/v2/users', {
      email,
      password,
      username,
      user_metadata,
